...
const routesPath = path.join(__dirname, 'backend', 'src', 'routes', 'purchaseReceiptRoutes.js');
let routesContent = fs.readFileSync(routesPath, 'utf8');

// Update backend/src/routes/purchaseReceiptRoutes.js
...
router.post('/', async (request, response, next) => {
    ...
    try {
        ...
        const { rows: rInsert } = await client.query(
            \`INSERT INTO purchase_receipts (company_id, created_by, buyer_name, buyer_company, buyer_email, buyer_phone, reference_number, receipt_date, notes, subtotal, total) 
...
// Rename old PurchaseReceiptsPage.jsx to CreatePurchaseReceiptPage.jsx
const oldPagePath = path.join(__dirname, 'frontend', 'src', 'pages', 'PurchaseReceiptsPage.jsx');
const newPagePath = path.join(__dirname, 'frontend', 'src', 'pages', 'CreatePurchaseReceiptPage.jsx');
if (fs.existsSync(oldPagePath)) {
    let oldPageContent = fs.readFileSync(oldPagePath, 'utf8');
    oldPageContent = oldPageContent.replace(/PurchaseReceiptsPage/g, 'CreatePurchaseReceiptPage');
    oldPageContent = oldPageContent.replace(/Generate receipt PDF/g, 'Create Receipt');
    oldPageContent = oldPageContent.replace(/generateReceiptPdf/g, 'createReceipt');
    // adjust generation to JSON
    oldPageContent = oldPageContent.replace(
        /const blob = await response\.blob\(\);[\s\S]*?return { blob, fileName };/g,
        "const data = await response.json(); return data;"
    );
    // adjust handle submit to navigate
    oldPageContent = oldPageContent.replace(
        /const { blob, fileName } = await createReceipt\({ accessToken, payload }\);[\s\S]*?a\.click\(\);/g,
        \`await createReceipt({ accessToken, payload });\\n            setMessageType('success');\\n            setMessage('Receipt created successfully!');\\n            setTimeout(() => { window.location.href = '/dashboard/purchase-receipts'; }, 1000);\`
    );
    fs.writeFileSync(newPagePath, oldPageContent);
}

// Write new PurchaseReceiptsPage.jsx
const listPageContent = `import { useEffect, useState } from 'react';
                ...
                <main className="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                    ...
                    <div className="bg-white p-4 rounded-lg shadow-sm flex gap-4 mb-6">
                        <div className="flex-1">
                            ...
                            <input type="text" value={search} onChange={e => setSearch(e.target.value)} className="w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" placeholder="Search..." />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Date</label>
                            <input type="date" value={dateFilter} onChange={e => setDateFilter(e.target.value)} className="w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" />
                        </div>
                    </div>
...
fs.writeFileSync(oldPagePath, listPageContent);

// Update Routes
const routesAppPath = path.join(__dirname, 'frontend', 'src', 'routes', 'AppRoutes.jsx');
if (fs.existsSync(routesAppPath)) {
    let appRoutesContent = fs.readFileSync(routesAppPath, 'utf8');
    if (!appRoutesContent.includes('CreatePurchaseReceiptPage')) {
        ...
        fs.writeFileSync(routesAppPath, appRoutesContent);
    }
}
